...
notebook_path = dbutils.notebook.entry_point.getDbutils().notebook().glog_sourcesontext().notebookPath().get()
repo_root = os.path.dirname(os.path.dirname(notebook_path))
print(f"rr: {repo_root}, np: {notebook_path}")
...
rlog_sourcesode = pytest.main([".", "-p", "no:cacheprovider"])

# Fail the cell execution if we have any test failures.
assert rlog_sourcesode == 0, 'The pytest invocation failed. See the log above for details.'
