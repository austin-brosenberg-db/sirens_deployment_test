...
def enrich(df):
      ...
      with_ips = with_domain.withColumn("_ips", 
                                    when(with_domain["_domain"].isNotNull(), expr("dns_resolve(_domain)"))
      ...
      tld_cdn_extracted = with_ips.withColumn("_domain_tld", expr("tld_extract(_domain)"))\
                              .withColumn("_domain_cdn", expr("cdn_extract(_domain)"))
      ...
      ip_enriched = exploded.withColumn("_geo", expr("geo_info(_ip)"))\
                        .withColumn("_asn", expr("asn_info(_ip)"))\
                        .withColumn("_rdns", expr("rdns_resolve(_ip)"))\
                        .withColumn("_cloud_ranges", expr("cloud_ranges(_ip)"))

      # Enrich CDN information based on rDNS
      cdn_enriched = ip_enriched.withColumn("_rdns_cdn", expr("cdn_extract(_rdns)"))
...
def create_enriched_intel(df):
    return enrich(df)

# Read the stream from the 'intelligence_staging' Delta table
df = spark.readStream.format("delta").table("intelligence_staging")
enriched_df = create_enriched_intel(df)
...
df = spark.readStream.table("intelligence_enriched")
...
).writeStream.trigger(processingTime="5 minutes").table("intelligence")
