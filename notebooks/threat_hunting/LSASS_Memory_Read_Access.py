...
# MAGIC %md
# MAGIC HNT-TA0007-T1087-001-100-LSASS_Memory_Read_access
...
datasetJSONPath = zipFile.extract(zipFile.namelist()[0], path="/Volumes/derek/sirens/sample_data")

# COMMAND ----------

datasetJSONPath = '/Volumes/derek/sirens/sample_data/empire_mimikatz_logonpasswords_2020-08-07103224.json'
df = spark.read.json(datasetJSONPath)
#display(df)
...
# MAGIC # Analytic I
# MAGIC
# MAGIC Look for non-system accounts getting a handle to and accessing lsass. Even though most adversaries might inject into a System process to blend in with most applications accessing LSASS, there are ocassions where adversaries do not elevate to System and use the available administrator rights from the user since that is the minimum requirement to access LSASS.
# MAGIC
# MAGIC |Data source|Event Provider|Relationship|Event|
# MAGIC |-----------|--------------|------------|-----|
# MAGIC |Process|Microsoft-Windows-Security-Auditing|Process accessed Process|4663|
# MAGIC |Process|Microsoft-Windows-Security-Auditing|Process requested_access Process|4656|

# COMMAND ----------


with TH.ThreatHunt(hunt_name) as lsass:
    @lsass.capture_output(command_name="analytic_1", description="looks for non-system accounts accessing lsass")
    def analytic_1(df):
        df = df.filter((F.lower("Channel") == 'security')  
                          & ((F.col('EventID') == 4663) | (F.col('EventID') == 4656)) 
                          & (F.col("ObjectName").rlike('lsass.exe'))
                          & (~F.col('SubjectUserName').endswith('.*$'))
                          ).select('@timestamp','Hostname','SubjectUserName','ProcessName','ObjectName','AccessMask','EventID')

        if not df.isEmpty():
            # add a risk score to the DataFrame row(s)                 
...
with TH.ThreatHunt(hunt_name) as lsass:
    @lsass.capture_output(command_name="analytic_2", description="Look for processes opening handles and accessing Lsass with potential dlls in memory ")
    def analytic_2(df):
...
# MAGIC %md
# MAGIC # Analytic III
# MAGIC Look for processes loading a few known DLLs loaded by tools like Mimikatz to interact with credentials.
# MAGIC
# MAGIC |Data source|Event Provider|Relationship|Event|
# MAGIC |-----------|--------------|------------|-----|
# MAGIC |Module|Microsoft-Windows-Sysmon/Operational|Process loaded Dll|7|
# MAGIC
# MAGIC
...
with TH.ThreatHunt(hunt_name) as lsass:
    @lsass.capture_output(command_name="analytic_3", description="Look for processes loading a few known DLLs loaded by tools")
    def analytic_3(df):
        df = df.filter(
        (F.col('Channel') == 'Microsoft-Windows-Sysmon/Operational')
        & (F.col('EventID') == 7)
        & (
            (F.col('ImageLoaded').rlike('.*samlib.dll'))
            | (F.col('ImageLoaded').rlike('.*vaultcli.dll'))
            | (F.col('ImageLoaded').rlike('.*hid.dll'))
            | (F.col('ImageLoaded').rlike('.*winscard.dll'))
            | (F.col('ImageLoaded').rlike('.*cryptdll.dll'))
        )
        & (F.col('@timestamp').between('2020-06-00 00:00:00.000','2020-08-20 00:00:00.000'))

        ).groupBy(F.col("ProcessGuid"), F.col("Image")).agg(F.count(F.col("ImageLoaded")).alias("ImagesLoaded"))
        
        return df
analytic_3_df = analytic_3(df)
display(analytic_3_df)

# COMMAND ----------

# MAGIC %md
# MAGIC # Analytic IV
# MAGIC Join processes opening a handle and accessing LSASS with potential DLLs loaded in memory and processes loading a few known DLLs loaded by tools like Mimikatz to interact with credentials.
# MAGIC
# MAGIC |Data source|Event Provider|Relationship|Event|
# MAGIC |-----------|--------------|------------|-----|
# MAGIC |Module|Microsoft-Windows-Sysmon/Operational|Process loaded Dll|7|
...
with TH.ThreatHunt(hunt_name) as lsass:
    @lsass.capture_output(command_name="analytic_4", description="Join processes opening a handle and accessing LSASS with potential DLLs")
    def analytic_4(df):
        imageloadDF_PY = df.filter(
            (F.col('Channel') == 'Microsoft-Windows-Sysmon/Operational')
            & (F.col('EventID') == 7)
            & (
                (F.col('ImageLoaded').rlike('.*samlib.dll'))
        ...
        df = imageloadDF_PY.join(processAccessDF_PY, on = imageloadDF_PY.ProcessGuid == processAccessDF_PY.SourceProcessGUID, how = "inner")
        
        return df
    
analytic_4_df = analytic_4(df)
display(analytic_4_df)
...
# MAGIC %md
# MAGIC # References
# MAGIC https://tyranidslair.blogspot.com/2017/10/bypassing-sacl-auditing-on-lsass.htmls
# MAGIC
